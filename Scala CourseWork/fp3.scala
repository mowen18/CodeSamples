...
object fp3:
  ...
  def member(a: Int, xs: List[Int]): Boolean =
    ...
    xs match
      case Nil => false
      case y::ys => if(y!=a) then member(a,ys) else true
  ...
  def allEqual(xs: List[Int]): Boolean =
    ...
    xs match
      case Nil => true
      case y::Nil => true
      case y::ys => if(y != ys.head) then false else allEqual(ys)
  ...
  def stringLengths(xs: List[String]): List[(String, Int)] =
    // TODO: Provide definition here.
   if (!xs.isEmpty) then xs.map(y => (y,y.length)) else Nil 
  ...
  def delete1[X](x: X, ys: List[X]): List[X] =
    ...
    ys match
      case Nil => Nil
      case z::zs => if(z!=x) then z::delete1(x,zs) else delete1(x,zs)
  ...
  def delete2[X](x: X, ys: List[X]): List[X] =
    // TODO: Provide definition here.
    if (!ys.isEmpty) then for(i <- ys if i != x) yield i else Nil
  ...
  def delete3[X](x: X, ys: List[X]): List[X] =
    // TODO: Provide definition here.
    if ys.isEmpty then Nil else ys.filter(z => z!=x)
  ...
  def removeDupes1[X](xs: List[X]): List[X] =
    ...
    xs match
      case Nil => Nil
      case y::Nil => List(y)
      case y::ys => if(y != ys.head) then y::removeDupes1(ys) else removeDupes1(ys)
  ...
  def removeDupes2[X](xs: List[X]): List[(Int, X)] =
    // TODO: Provide definition here.   
    def rmHelper [X] (xs:List[X], num:Int,elm:X) : List[(Int,X)] = 
      xs match
        case Nil => if (elm != Nil) then (num,elm)::Nil else Nil
        case y::ys => if (y!=elm) then (num,elm)::rmHelper(ys,1,y) else rmHelper(ys,num+1,elm)
    {
      if (xs.isEmpty) then Nil else rmHelper(xs.tail,1,xs.head)
    }
  ...
  def splitAt[X](n: Int, xs: List[X]): (List[X], List[X]) =
    // TODO: Provide definition here.
    (n,xs) match
      case (i,y::ys) => { val(one,two) = splitAt(i-1,ys) 
      if (i<=0) then (one,y::two) else (y::one,two)}
      case (_,Nil) => (Nil,Nil)
  ...
  def allDistinct(xs: List[Int]): Boolean =
    ...
    xs match
      case Nil => true
      case y::ys => if(!member(y,ys)) then allDistinct(ys) else false
