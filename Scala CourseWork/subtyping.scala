...
object subtyping:
  ...
  def observeCounter(f: Counter => Unit): Int = {
    ...
    class CountTracker extends Counter {
    private var n = 0
    private var s = 0
    override def increment () = { n = n + 1; s +=1}
    override def decrement () = { n = n + 1; s +=1}
    def doSum () : Int = s
  }
    val ctrack = new CountTracker
    f(ctrack)
    ctrack.doSum()
  }
  ...
  def observeCounterList(f: List[Counter] => Unit): List[Int] = {
    ...
    val t1 = new CountTracker
    val t2 = new CountTracker
    val t3 = new CountTracker
    f(List(t1,t2,t3))
    List(t1.doSum(),t2.doSum(),t3.doSum())
}
  ...
  def observeCounterArray(f: Array[Counter] => Unit): Array[Int] = {
    ...
    val t3 = new CountTracker
    f(Array(t1,t2,t3))
    Array(t1.doSum(),t2.doSum(),t3.doSum())
}
