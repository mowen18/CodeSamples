...
SELECT AName,TimeToFeed FROM Animal where ACategory='rare';

--(ii)Which animal(s) have a `time to feed' larger than every rare animal? Give the id and name of the animal.
SELECT AID, AName FROM ANIMAL a1 WHERE a1.TimeToFeed >= (SELECT max(TimeToFeed) FROM Animal WHERE ACategory = 'rare');

--(iii)Name zookeepers handling at least 4 animals.
SELECT ZName FROM Zookeeper WHERE Zid = (SELECT ZOOKEEPID FROM Handles GROUP BY ZOOKEEPID having count(AnimalID) >=4);
...
SELECT Zname, HourlyRate FROM ZooKeeper WHERE Hourlyrate >= (select max(Hourlyrate) FROM Zookeeper);
